...
extern G4int gNStrips;
extern G4int gTotalNScintillators;
  ...
  : G4VSensitiveDetector(name), fNameSD(name), fHitsCollection(nullptr), fHCID(-1)
{
  ...
  fEdep.resize(gTotalNScintillators,0);
  fEweightedx.resize(gTotalNScintillators,0);
  fEweightedy.resize(gTotalNScintillators,0);
  fEweightedz.resize(gTotalNScintillators,0);
  fEweightedt.resize(gTotalNScintillators,0);
  ...
  if (gSaveStepLevel == true){
    ...
    fStepEdep.resize(gTotalNScintillators);
  
    fPreStepx.resize(gTotalNScintillators);
    fPreStepy.resize(gTotalNScintillators);
    fPreStepz.resize(gTotalNScintillators);
    fPreStept.resize(gTotalNScintillators);
  
    fPostStepx.resize(gTotalNScintillators);
    fPostStepy.resize(gTotalNScintillators);
    fPostStepz.resize(gTotalNScintillators);
    fPostStept.resize(gTotalNScintillators);

    fParticlePx.resize(gTotalNScintillators);
    fParticlePy.resize(gTotalNScintillators);
    fParticlePz.resize(gTotalNScintillators);
    fParticleTrackID.resize(gTotalNScintillators);
    fParticleParentID.resize(gTotalNScintillators);
    fParticleCharge.resize(gTotalNScintillators);
    fParticleMass.resize(gTotalNScintillators);
    fParticlePDGID.resize(gTotalNScintillators);
  }
}
...
void B5EmCalorimeterSD::EndOfEvent(G4HCofThisEvent* hce){
  for (int i = 0; i < gTotalNScintillators; i++){
    ...
    auto hit = (B5EmCalorimeterHit*) ((hce -> GetHC(fHCID)) -> GetHit(CurrentHitID));
    G4int layerID = (G4int) i/gNStrips;
    G4int segmentID = (G4int) i%gNStrips;
